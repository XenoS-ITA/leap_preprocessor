if not leap then leap={}end;if not leap.deserialize then leap.deserialize=function(a)local b=_type(a)if b~="table"or not a.__type then if b=="table"then local c={}for d,e in pairs(a)do c[d]=leap.deserialize(e)end;return c else return a end end;local f=_G[a.__type]if not f then error("Class '"..a.__type.."' not found",2)end;f.__skipNextConstructor=true;local g=f()if g.deserialize then g:deserialize(a)else for d,e in pairs(a)do if d~="__type"then g[d]=leap.deserialize(e)end end end;return g end end;if not leap.serialize then leap.serialize=function(a)local b=_type(a)if b~="table"then return a end;if a.serialize then local h=a:serialize()if not h then return nil end;for d,e in pairs(h)do h[d]=leap.serialize(e)end;h.__type=a.__type;return h end;local c={}for d,e in pairs(a)do c[d]=leap.serialize(e)end;return c end end;if not leap.fsignature then leap.fsignature=function(i)if _type(i)~="function"then error("leap.fsignature: passed argument must be a function, but got ".._type(i),2)end;if not __leap__introspection then return nil end;return __leap__introspection[i]end end;if not leap.registerfunc then leap.registerfunc=function(i,j)if not __leap__introspection then __leap__introspection={}end;__leap__introspection[i]=j;return i end end;if not _type then _type=type;type=function(k)local l=_type(k)if l=="table"and k.__type then return k.__type else return l end end end;if not __leap_call_kargs then __leap_call_kargs=function(a,b,...)local c={...}local d=leap.fsignature(a)if not d then error("leap: "..a.." cant accept kargs as it has no metadata (probably not processed by leap)",2)end;for e,f in pairs(b)do for g,h in pairs(d.args)do if h.name==e then if c[g]then local i=string.format("leap: argument '%s' for function '%s' was provided both positionally (%s) and as a keyword (%s)",e,d.name,tostring(c[g]),tostring(f))error(i,2)end;c[g]=f;break end end end;return table.unpack(c)end end;if not _leap_internal_using_operator then _leap_internal_using_operator=function(a,b,...)local c=a;a=_G[a]if type(a)~="string"then error("using operator expects a filter, got "..type(a),2)end;local d=b.ddd;b.ddd=nil;local e={...}local f,g=pcall(function()load(a,"@"..c,"t",setmetatable(b,{__index=_G}))()(table.unpack(e),d and table.unpack(d))end)if not f then error(g,2)end end end;if not _leap_internal_in_operator then _leap_internal_in_operator=function(a,b)local c=type(b)if c=="table"then if table.type(b)=="array"then for d,e in pairs(b)do if e==a then return true end end elseif table.type(b)=="hash"then for d,e in pairs(b)do if d==a then return true end end else for d,e in pairs(b)do if e==a or d==a then return true end end end elseif c=="string"then return b:find(a)else error("in operator: unsupported type "..c)end;return false end end;if not _leap_internal_classBuilder then local a={__index=function(self,b)local c=self.parent.__prototype[b]if type(c)=="function"then local d=self.cache[b]if not d then d=function(e,...)return c(e.obj,...)end;self.cache[b]=d end;return d else return c end end,__call=function(self,...)if self.parent.__prototype.constructor then local f=self.obj.super;self.obj.super=_leap_internal_class_makeSuper(self.obj,self.parent.__prototype.__parent)local g=self.parent.__prototype.constructor(self.obj,...)self.obj.super=f;return g else error("leap: super class has no constructor",2)end end,__newindex=function()error("cannot assign to super",2)end}local function h(i,j)if type(i)~="table"then return i end;if j and j[i]then return j[i]end;local k={}j=j or{}j[i]=k;for l,m in next,i do local n=type(l)=="table"and h(l,j)or l;local o=type(m)=="table"and h(m,j)or m;k[n]=o end;return k end;function _leap_internal_class_makeSuper(p,q)return setmetatable({cache={},parent=q,obj=p},a)end;_leap_internal_classBuilder=function(r,s,t)s._leap_internal_decorators={}if not t then error("ExtendingNotDefined: "..r.." tried to extend a class that is not defined",2)end;local u=t.__prototype;if u then setmetatable(s,{__index=u})s.__parent=t end;local v={}local w=1;for l,m in next,s do if _type(m)=="table"and l:sub(1,5)~="_leap"then v[w]=l;w=w+1 end end;local x={__index=s,__gc=function(self)if self.destructor then self:destructor()end end,__tostring=function(self)if self.toString then return self:toString()else local y=""for l,m in pairs(self)do if l~="__type"and l~="super"and l:sub(1,5)~="_leap"then if _type(m)=="table"then if getmetatable(m)then m=tostring(m)else m=json.encode(m)end end;y=y..l..": "..tostring(m)..", "end end;y=y:sub(1,-3)return"<"..self.__type..":"..("%p"):format(self).."> "..y end end}_G[r]=setmetatable({__type=r,__prototype=s},{__newindex=function(self,l,m)if l:sub(1,2)=="__"then rawset(self,l,m)else error("attempt to assign class property '"..l.."' directly, please instantiate the class before assigning any properties",2)end end,__call=function(self,...)local z=self.__prototype;local p={__type=self.__type}for A=1,#v do local b=v[A]p[b]=h(z[b])end;setmetatable(p,x)p.super=_leap_internal_class_makeSuper(p,s.__parent)for e,B in pairs(p._leap_internal_decorators)do local C=p[B.name]local D=function(...)return C(p,...)end;leap.registerfunc(D,leap.fsignature(C))p[B.name]=_G[B.decoratorName](p,D,table.unpack(B.args))or C end;if not self.__skipNextConstructor then if p.constructor then p:constructor(...)elseif u then t(...)end end;self.__skipNextConstructor=nil;return p end})end;_leap_internal_classBuilder("Error",{constructor=function(self,E)self.message=E end,toString=function(self)return type(self)..": "..self.message end},{})end;if not _leap_internal_is_operator then _leap_internal_is_operator=function(p,F)if not p or not F then return false end;if _type(p)~="table"then error("leap.is_operator: #1 passed argument must be a class instance, but got ".._type(p),2)end;if _type(F)~="table"then error("leap.is_operator: #2 passed argument must be a class, but got ".._type(F),2)end;if p.__prototype then error("leap.is_operator: #1 passed argument must be a class instance, but got class",2)end;local G=p;while G and G.__type~=F.__type do if G.super then G=G.super.parent else return false end end;return true end end;  
