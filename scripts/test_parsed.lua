if not leap then leap={}end;if not leap.deserialize then leap.deserialize=function(a)local b=_type(a)if b~="table"or not a.__type then if b=="table"then local c={}for d,e in pairs(a)do c[d]=leap.deserialize(e)end;return c else return a end end;local f=_G[a.__type]if not f then error("Class '"..a.__type.."' not found",2)end;f.__skipNextConstructor=true;local g=f()if g.deserialize then g:deserialize(a)else for d,e in pairs(a)do if d~="__type"then g[d]=leap.deserialize(e)end end end;return g end end;if not leap.serialize then leap.serialize=function(a)local b=_type(a)if b~="table"then return a end;if type(a)=="table"then local h={}for d,e in pairs(a)do h[d]=leap.serialize(e)end;return h end;if a.serialize then local i=a:serialize()if not i then return nil end;if type(i)~="table"then error("leap.serialize: custom serialize method must return a table",2)end;for d,e in pairs(i)do i[d]=leap.serialize(e)end;i.__type=a.__type;return i end;local c={}for d,e in pairs(a)do if d~="__stack"then local j=false;if a.__ignoreList then for k,l in pairs(a.__ignoreList)do if d==l then j=true;break end end end;if not j then c[d]=leap.serialize(e)end end end;return c end end;if not skipSerialize then skipSerialize=function(m,n)if _type(m)~="table"then error("skipSerialize: #1 passed argument must be a class, but got "..type(m),2)end;m.__prototype.__ignoreList=n end end;if not leap.fsignature then leap.fsignature=function(o)if _type(o)~="function"then error("leap.fsignature: passed argument must be a function, but got ".._type(o),2)end;if not __leap__introspection then return nil end;return __leap__introspection[o]end end;if not leap.registerfunc then leap.registerfunc=function(o,p)if not __leap__introspection then __leap__introspection=setmetatable({},{__mode="k"})end;__leap__introspection[o]=p;return o end end;if not leap.minimal then leap.minimal=false end;if not _type then _type=type;type=function(q)local r=_type(q)if r=="table"and q.__type then return q.__type else return r end end end;if not __leap_call_kargs then __leap_call_kargs=function(a,b,...)local c={...}local d=leap.fsignature(a)if not d then error("leap: "..a.." cant accept kargs as it has no metadata (probably not processed by leap)",2)end;for e,f in pairs(b)do for g,h in pairs(d.args)do if h.name==e then if c[g]then local i=string.format("leap: argument '%s' for function '%s' was provided both positionally (%s) and as a keyword (%s)",e,d.name,tostring(c[g]),tostring(f))error(i,2)end;c[g]=f;break end end end;return table.unpack(c)end end;if not _leap_internal_using_operator then _leap_internal_using_operator=function(a,b,...)local c=a;a=_G[a]if type(a)~="string"then error("using operator expects a filter, got "..type(a),2)end;local d=b.ddd;b.ddd=nil;local e={...}local f,g=pcall(function()load(a,"@"..c,"t",setmetatable(b,{__index=_G}))()(table.unpack(e),d and table.unpack(d))end)if not f then error(g,2)end end end;if not _leap_internal_in_operator then _leap_internal_in_operator=function(a,b)local c=type(b)if c=="table"then if table.type(b)=="array"then for d,e in pairs(b)do if e==a then return true end end elseif table.type(b)=="hash"then for d,e in pairs(b)do if d==a then return true end end else for d,e in pairs(b)do if e==a or d==a then return true end end end elseif c=="string"then return b:find(a)else error("in operator: unsupported type "..c)end;return false end end;if not _leap_internal_classBuilder then local a=function(b)local c=b.__stack[#b.__stack]return c and c.__prototype end;local d=function(b)local c=b.__stack[#b.__stack]return c and c.__type end;local e={}local f=function(b,g)if not g.__parent then table.insert(b.__stack,e)else table.insert(b.__stack,g.__parent)end end;local h=function(b)local i=table.remove(b.__stack)end;if not table.clone then table.clone=function(j)local k={}for l,m in pairs(j)do k[l]=m end;return k end end;_leap_internal_deepcopy=function(j,n)if type(j)~="table"then return j end;n=n or{}if n[j]then return n[j]end;local o=table.clone(j)n[j]=o;for l,m in next,j do if type(m)=="table"then o[l]=_leap_internal_deepcopy(m,n)end end;return o end;local p={__index=function(self,q)if q=="__type"then return d(self.obj)else local g=a(self.obj)if not g or next(g)==nil then return nil end;local r=g[q]if type(r)=="function"then local s=leap.fsignature(r)return leap.registerfunc(function(t,...)f(self.obj,g)local u=table.pack(r(self.obj,...))h(self.obj)return table.unpack(u)end,s)else return r end end end,__call=function(self,...)local v=self.constructor;if v then return v(...)end end}function _leap_internal_class_makeSuper(b)return setmetatable({obj=b},p)end;_leap_internal_classBuilder=function(w,x,y)x._leap_internal_decorators={}if not y then error("ExtendingNotDefined: "..w.." tried to extend a class that is not defined",2)end;if y.__prototype then x.__parent=y end;local z={__cache={},__newindex=function(A,l,m)rawset(A,l,m)getmetatable(A).__cache[l]=nil end}setmetatable(x,z)local B={}local C=1;for l,m in next,x do if _type(m)=="table"and l:sub(1,5)~="_leap"then B[C]=l;C=C+1 end end;local D={__index=function(self,q)local E=getmetatable(x).__cache;local g=x;local r=g[q]if not E[q]then while g do if g[q]~=nil then r=g[q]break end;if not g.__parent then return nil end;g=g.__parent.__prototype end;if type(r)=="function"then local s=leap.fsignature(r)E[q]=leap.registerfunc(function(t,...)if not t then error("leap: You need to pass self when calling a class method",2)end;f(t,g)local u=table.pack(r(t,...))h(t)return table.unpack(u)end,s)else E[q]={proto=g}end else end;if type(E[q])=="function"then return E[q]else local g=E[q].proto;return g[q]end end,__gc=function(self)if self.destructor then self:destructor()end end,__tostring=function(self)if self.toString then return self:toString()else local F=""if not leap.minimal then for l,m in pairs(self)do if l~="super"and l:sub(1,5)~="_leap"and l:sub(1,2)~="__"then local G=_type(m)if G~="function"then local H=m;if G=="table"then local I=getmetatable(m)if not I or not I.__tostring then H=json.encode(m)end end;if G=="string"then H='"'..m..'"'end;F=F..l..": "..tostring(H)..", "end end end;F=F:sub(1,-3)end;return"<"..self.__type..":"..("%p"):format(self).."> "..F end end}_G[w]=setmetatable({__type=w,__prototype=x},{__newindex=function(self,l,m)if l:sub(1,2)=="__"then rawset(self,l,m)else error("attempt to assign class property '"..l.."' directly, please instantiate the class before assigning any properties",2)end end,__call=function(self,...)local b={__type=self.__type,__stack={}}for J=1,#B do local q=B[J]b[q]=_leap_internal_deepcopy(self.__prototype[q])end;setmetatable(b,D)local K=self.__prototype.__parent;if K then b.super=_leap_internal_class_makeSuper(b)end;for t,L in pairs(b._leap_internal_decorators)do local M=b[L.name]local N=function(...)return M(b,...)end;leap.registerfunc(N,leap.fsignature(M))b[L.name]=_G[L.decoratorName](b,N,table.unpack(L.args))or M end;if not self.__skipNextConstructor then local O=b.constructor;if O then O(b,...)end end;self.__skipNextConstructor=nil;return b end})end;_leap_internal_classBuilder("Error",{constructor=function(self,P)self.message=P end,toString=function(self)return type(self)..": "..self.message end},{})end;if not _leap_internal_is_operator then _leap_internal_is_operator=function(b,Q)if not b or not Q then return false end;if _type(b)~="table"then error("leap.is_operator: #1 passed argument must be a class instance, but got ".._type(b),2)end;if _type(Q)~="table"then error("leap.is_operator: #2 passed argument must be a class, but got ".._type(Q),2)end;if b.__prototype then error("leap.is_operator: #1 passed argument must be a class instance, but got class",2)end;local R=b;while R and R.__type~=Q.__type do if R.super then R=R.super.parent else return false end end;return true end end;  
